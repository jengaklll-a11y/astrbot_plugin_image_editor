...
## 功能特性
- **多风格转换**：内置改图预设模板。
- **自定义生成**：使用 `/jm <提示词>` 指令，可以完全自定义 Prompt 进行创作。
...
- **防止冗杂**：添加一个配置开关，可以避免日常聊天中误触那些预设的关键词（其实是为了不跟相同功能的插件起冲突）
## 安装与配置
...
| 命令 | 功能说明 |
| :--- | :--- |
| `/jm添加key <key1>...` | 添加一个或多个API密钥 |
| `/jmkey列表` | 查看API密钥列表 |
| `/jm删除key <序号\|all>` | 删除API密钥 |
| `/jm增加用户次数 <QQ号> <次数>` | 为用户增加使用次数 |
| `/jm减少用户次数 <QQ号> <次数>` | 为用户增加使用次数 |
| `/jm增加群组次数 <Q群号> <次数>` | 为群组增加使用次数 |
| `/jm减少群组次数 <Q群号> <次数>` | 为群组增加使用次数 |
| `/jm查询次数 <QQ号>` | 查询指定用户剩余次数 |
